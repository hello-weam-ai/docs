...
<Note>
Replace `https://your-domain.com` with your actual domain before running this script.
</Note>
...
sed -i.bak "s|http://localhost:9089|$CUSTOM_DOMAIN|g" .env
...
cat <<EOL | sudo tee /etc/nginx/sites-available/weam
server {
    listen 80;
    server_name $DOMAIN;
...
sudo ln -sf /etc/nginx/sites-available/weam /etc/nginx/sites-enabled/
...
<Note>
Replace `https://your-domain.com` with your actual domain before using this script.
</Note>
...
# Create Nginx configuration for WeamAI
cat <<EOL | sudo tee /etc/nginx/sites-available/weam
server {
    listen 80;
    server_name $DOMAIN;
